...
#' @param ids A vector of two typed individuals in `x`. By default, the typed
#'   members of `x` are included, but note that this gives an error if the
#'   number of such individuals is not 2.
...
quickLR = function(x, ids = typedMembers(x), test = c("pat", "sib", "half")) {
  ...
  res$likelihoodsPerMarker = res$likelihoodsPerMarker[, -n, drop = FALSE]
  res
}
